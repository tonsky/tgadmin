...
(defn check-stop-words [message]
  (when-some [s (:text message)]
    (when-some [words (not-empty
                        (concat
                          (re-seq #"(?uUi)\b(?:сотрудничеств|сфер|выплат|направлени|заработ|доход|доллар|средств|деньг|личк|русло|актив|работ|команд|обучени|юмор|улыб|мудак|говн|курс|помощь|пиш|срочн)[а-я]*\b" s)
                          (re-seq #"(?uUi)\b(?:лс)\b" s)
